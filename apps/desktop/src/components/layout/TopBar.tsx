...
import { TypingMetrics } from '../../lib/tauri'
import { useAuthStore } from '../../core/store/authStore'
import { useUpdater } from '../../hooks/useUpdater'
import { syncService } from '../../core/syncService'
import { getProgressToNextRank } from '../../core/rankSystem'
...
export const TopBar = ({ metrics = { raw_wpm: 0, adjusted_wpm: 0, accuracy: 100, consistency: 100, is_bot: false, cheat_flags: '' }, mode, onAnalyticsClick, onSocialClick }: TopBarProps) => {
  const { user, setAuthenticated, logout } = useAuthStore()
  const [showSettings, setShowSettings] = useState(false)
  const { manualCheck, checking } = useUpdater()
  ...
  return (
    <div className="fixed top-5 left-1/2 -translate-x-1/2 w-[90%] max-w-[1400px] h-16 flex justify-between items-center px-8 z-50 transition-all duration-300 glass-perfect shadow-[0_20px_50px_rgba(0,0,0,0.3)]" data-tauri-drag-region>
      ...
      <div className="top-bar-right flex items-center gap-2">
        ...
        {/* Social Button */}
        <button className="icon-btn" onClick={onSocialClick} title="Community & Profile">
